...
/*The intention in using particularly 0,
 * ist to not create a Struct unintentionally.*/
...
void deserilize_row(void* source, Row* destination){
  /*
  printf("im deserilizing\n");
  printf("these are the values: \n");
  printf("ID off:%d , ID size:%d \n", ID_OFFSET, ID_SIZE);
  printf("USERNAME off:%d , USERNAME  size:%d \n", USERNAME_OFFSET, USERNAME_SIZE);
  printf("EMIAL off:%d , EMAILsize:%d \n", EMAIL_OFFSET, EMAIL_SIZE);
  */
  memcpy( &(destination->id) , source + ID_OFFSET , ID_SIZE );
  printf("deserilizing ID done.\n");
  memcpy( &(destination->username) , source + USERNAME_OFFSET , USERNAME_SIZE);
  printf("deserilizing username done.\n");
  memcpy( &(destination->email) , source + EMAIL_OFFSET , EMAIL_SIZE );
  printf("deserilizing email done.\n");
}
...
ExecuteResult execute_select(Statement* statement, Table* table){
  ...
  for ( i = 0 ; i<table->num_rows ; i++ ) {
    void* src = row_slot(table,i);
    printf("%p \n", src);
    deserilize_row(src,&row);
    print_row(&row);
  }
